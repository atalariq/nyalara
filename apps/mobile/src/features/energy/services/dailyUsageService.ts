...
async function readDraftQueue(userId: string): Promise<PendingUsageDraft[]> {
  const raw = await AsyncStorage.getItem(getDraftQueueKey(userId))
...
async function writeDraftQueue(userId: string, drafts: PendingUsageDraft[]) {
  await AsyncStorage.setItem(getDraftQueueKey(userId), JSON.stringify(drafts))
}
...
async function flushQueuedDrafts(userId: string) {
  const drafts = await readDraftQueue(userId)

  if (drafts.length === 0) {
    return
  }

  const remaining: PendingUsageDraft[] = []
  ...
  await writeDraftQueue(userId, remaining)
}

async function getHistoryRange(userId: string, days: number) {
  await flushQueuedDrafts(userId)
...
export const dailyUsageService = {
  ...
  async accumulateDeviceUsage(
    ...
    const onlineDraft = createUsageDraft(date, deviceId, record.durationMinutes, false)
